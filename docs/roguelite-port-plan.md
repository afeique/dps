...
| **BO** — boss chassis | 🟢 **mostly DONE** | bosses now have **multi-phase escalation** (calm→frenzy@66%→rage@33%, each with fire-rate ramp, ring burst, visual cue + boss-roar SFX) AND a **weak-point part chassis** (`spawn_boss_parts`: a ring of destructible turrets that shield the core via `CoreShielded` until cleared; `update_core_shield` + the gate in `bullet_hits_enemy`; `update_boss_parts` positions them). Remaining: scripted per-phase attack *patterns* + a part-aware boss healthbar UI. |
| **X** — run configurator / adaptive difficulty | 🟢 **run-config DONE** | a pre-run **difficulty** (Normal/Hard/Brutal → ×1/1.5/2.2 enemy HP) is picked in the BUILD screen's STATS tab, persisted on `Meta.difficulty`, and applied per fresh enemy by `difficulty::apply_difficulty` (`systems/difficulty.rs`). The *adaptive* (auto-scaling-to-performance) director is the remaining, forward-looking half. |
